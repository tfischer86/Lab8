describe('Basic user flow for SPA ', () => {
  ...
  it('Test11: Clicking the back button once should bring the user back to the home page', async () => {
    await page.goBack();
    expect(page.url()).toMatch(/127\.0\.0\.1:5500\/$/);
  });
  ...
  it('Test15: On the second Entry page, the title should be "Entry 2"', async () => {
    const title = await page.$eval('header > h1', header => header.innerText);
    expect(title).toEqual('Entry 2');
  });
  ...
  it('Test17: Clicking the forward button once should bring the user to second entry page', async () => {
    await page.goForward();
    expect(page.url()).toMatch(/#entry2$/);
  });

  // create your own test 18
  it('Test18: Clicking the title should bring the user back to the home page', async () => {
    await page.click('header > h1');
    expect(page.url()).toMatch(/127\.0\.0\.1:5500\/$/);
  });

  // create your own test 19
  it('Test19: After clicking the title to get to the home page, check the title is "Journal Entries"', async () => {
    const title = await page.$eval('header > h1', header => header.innerText);
    expect(title).toEqual('Journal Entries');
  });

  // create your own test 20
  it('Test20: After clicking the back button twice, the user should be at the home page', async () => {
    await page.goBack();
    await page.goBack();
    expect(page.url()).toMatch(/127\.0\.0\.1:5500\/$/);
    const title = await page.$eval('header > h1', header => header.innerText);
